...
bool rechercherLigne(bool parLaGauche)
{
  ...
  while (1)
  {
    ...
    piloterMoteur(MOTEUR_GAUCHE, AVANT_GAUCHE, VITESSE_RETOUR_LIGNE);
    piloterMoteur(MOTEUR_DROIT,  AVANT_DROIT,  VITESSE_RETOUR_LIGNE);
  ...
  // Directions inversées par rapport à avant
  if (parLaGauche)
  {
    piloterMoteur(MOTEUR_GAUCHE, AVANT_GAUCHE, 28);  // O1 → tourne gauche
    piloterMoteur(MOTEUR_DROIT,  ARRET,        0);
  }
  else
  {
    piloterMoteur(MOTEUR_GAUCHE, ARRET,       0);     // O2 → tourne droite
    piloterMoteur(MOTEUR_DROIT,  AVANT_DROIT, 28);
  }
